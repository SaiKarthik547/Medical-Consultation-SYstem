...
export default function DoctorHistoryPage() {
    ...
    const fetchHistory = async () => {
        try {
            // We can reuse the main consultations endpoint which filters by role "DOCTOR".
            // Ideally we filter by status too, e.g. status=COMPLETED
            // But getting all assigned is also fine for "History".
            const res = await fetch('/api/consultations');
            if (res.ok) {
                const data = await res.json();
                // Client side filter for Completed/Cancelled
            ...
            <div className="bg-white shadow-sm border-b">
                <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <Link href="/doctor" className="text-gray-500 hover:text-gray-900">
                            ← Back to Dashboard
                        </Link>
